...
// import image from '@/assets/images/defalutMaleImage.png'
import image from '@/assets/svgs/defaultMaleIcon.svg';
  ...
  return (
    <div className='flex flex-col my-2 w-1/2'>
      <div className='flex items-center gap-5'>
        <ProfileImage />
        <div>
          <span className='font-daeam'>chyeonRyong</span>{' '}
          <span className='font-her'>* 2일전</span>
        </div>
      </div>
      <div>
        <p className='my-5 px-3'>
          ...
          이렇게 안했을까 싶더라 내가 늦게 실천 하는거 일수도 있는데 혹시나
          별차이 없을꺼라고 생각하는 덬도 있을까바 실제 경험해서 적어봤어~~ 요즘
          이것 저것 너무 많이 오르는데 조금이라도 아껴보자~~!!
        </p>
      </div>
      <img
      ...
      <div className='flex items-center my-2 gap-2'>
        {isLike ? (
          <img className='cursor-pointer' src={unLikeIcon} alt='' />
        ) : (
          <img className='cursor-pointer' src={likeIcon} alt='' />
        )}
